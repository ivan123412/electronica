...
if (isset($_GET['query']) && !empty($_GET['query'])) {
    ...
    $stmt = $conexion->prepare("SELECT * FROM inventario WHERE nombre_producto LIKE :query AND (tipo = 'computadora' OR tipo = 'television') ORDER BY precio " . ($orden === 'desc' ? 'DESC' : 'ASC'));
    $stmt->execute([':query' => '%' . $query . '%']);
} else {
    $stmt = $conexion->prepare("SELECT * FROM inventario WHERE tipo IN ('electrodomestico', 'television') ORDER BY precio " . ($orden === 'desc' ? 'DESC' : 'ASC'));
    $stmt->execute();
}
...
<head>
    ...
    <title>Electrodomesticos y Televisores - Tienda de Electrónica</title>
...
<h2 class="titulo">Electrodomesticos y Televisores</h2>
