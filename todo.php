...
if (isset($_GET['query']) && !empty($_GET['query'])) {
    ...
    $stmt = $conexion->prepare("SELECT * FROM inventario WHERE nombre_producto LIKE :query ORDER BY precio " . ($orden === 'desc' ? 'DESC' : 'ASC'));
    $stmt->execute([':query' => '%' . $query . '%']);
} else {
    $stmt = $conexion->prepare("SELECT * FROM inventario ORDER BY precio " . ($orden === 'desc' ? 'DESC' : 'ASC'));
    $stmt->execute();
}
...
<head>
    ...
    <title>Productos - Tienda de Electrónica</title>
